...
from extract_predominant_melody import extract_from_midi_to_midi, process_folder
...
def instruments(events):
  insts = [128 if item.instrument == 'drum' else int(item.instrument) for item in events[1:-1] if item.name in ['Note', 'Melody']]
